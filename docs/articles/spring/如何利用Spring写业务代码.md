...
A(开始)-->B[下单]
B --> C{订单类型}
C -->|type=recharge| D[充值]:::someclass
C -->|type=withdraw| E[提现]:::someclass
C -->|type=transfer| F[转账]:::someclass
D -->G[通道]
E -->G[通道]
F -->G[通道]
G -->H(结束)
...
C -->|type=withdraw| E[提现]:::someclass
C -->|type=transfer| F[转账]:::someclass
C -->|type=frozen| I[冻结]:::someclass
C -->|type=unfrozen| J[解冻]:::someclass
D -->G[通道]
E -->G[通道]
F -->G[通道]
G -->H(结束)
...
C -->|type=frozen| I[冻结]:::someclass
C -->|type=unfrozen| J[解冻]:::someclass
C -->|type=purchase| K[申购]:::someclass
C -->|type=redeem| L[赎回]:::someclass
D -->G[支付通道]
E -->G[支付通道]
F -->G[支付通道]
K -->M[基金通道]
...
E -->G[支付通道]
F -->G[支付通道]
G -->|channel=alipay|N[支付宝]:::someclass
G -->|channel=wepay|O[微信]:::someclass
G -->|channel=bankpay|P[银行]:::someclass
N -->H(结束)
O -->H(结束) 
P -->H(结束) 
...
完成 -->|channel=alipay|送话费[送话费]:::someclass
完成 -->|channel=wepay|发红包[发红包]:::someclass
完成 -->|channel=bankpay|抽奖[抽奖]:::someclass

送话费 -->抽奖[抽奖]
发红包 -->抽奖[抽奖] 

抽奖 -->送积分[送积分]:::someclass
送积分 -->结束[结束]
```
...
监听充值成功事件 -->送积分[送积分]:::someclass
送积分 -->抽奖[抽奖]:::someclass
抽奖 -->|channel=alipay|送话费[送话费]:::someclass
抽奖 -->|channel=wepay|发红包[发红包]:::someclass
送话费 -->结束[结束]
发红包 -->结束[结束]
```
?> 可以抽象事件模型：
